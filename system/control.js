...
const group = async (ctx, event, eventType) => {
    try {
        ...
        const messages = {
            add: `✨ مرحباً بك في عالم الدشاش ${users} ✨
نورت الجروب 💖
${authorTag === users ? "" : `\n➤ بواسطة: ${authorTag}`}`,
...
${authorTag === users ? "" : `\n➤ بواسطة: ${authorTag}`}`,
            ...
            promote: `👑 مبروك ${users}
تمت ترقيتك أدمن في عالم الدشاش 🔥
➤ بواسطة: ${authorTag}`,

            demote: `📌 ${users}
رجعت عضو عادي في عالم الدشاش 🙂
➤ بواسطة: ${authorTag}`
        };
        ...
        if (global.db.groups[event.chat].noWelcome === true)
            return 9999;
